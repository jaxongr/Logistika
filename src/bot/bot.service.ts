...
<b>Qaysi rol bilan ro'yxatdan o'tasiz?</b>
      ...
      if (adminUsers.includes(user.id) || userRole?.role === 'admin') {
        ...
        const companyBalance = Array.from(this.userBalances.values())
...
📢 Yangi tizim tayyor bo'lgach sizga xabar beramiz!
  ...
  // Show real balance overview (not virtual)
...
📤 <b>Jami ishlatilgan:</b> ${totalDeductions.toLocaleString()} so'm

📈 <b>Oxirgi 5 ta operatsiya:</b>
${balanceHistory.slice(-5).reverse().map((t, i) =>
  `${i + 1}. ${t.type === 'top_up' ? '💰' : '📤'} ${t.amount.toLocaleString()} so'm
     📝 ${t.description || (t.type === 'top_up' ? 'Balans to\'ldirish' : 'Chiqarim')}
     📅 ${new Date(t.date).toLocaleDateString('uz-UZ')}`
).join('\n\n') || '• Operatsiyalar yo\'q'}

${currentBalance > 0 ? '✅ Balansingiz mavjud!' : '⏳ Balansni to\'ldiring'}
    `;
  ...
  private async handleCargoCompletion(ctx: any, cargoId: string) {
    ...
    if (cargo) {
      ...
      if (this.customerOrderHistory.has(cargo.userId)) {
        const customerOrders = this.customerOrderHistory.get(cargo.userId)!;
        const orderIndex = customerOrders.findIndex(order => order.id === cargoId);
        if (orderIndex !== -1) {
          customerOrders[orderIndex].status = 'completed';
          customerOrders[orderIndex].completedDate = new Date().toISOString();
        }
      }
    ...
    await this.safeAnswerCallback(ctx, '🎉 Buyurtma bajarildi!');

    const currentBalance = await this.getUserBalance(driverId);
    const message = `
...
💡 <b>Balans to'ldirish:</b> Admin orqali yoki to'lov tizimi orqali
    `;
...
📞 Yuk egasi bilan bog'lanib batafsil ma'lumot oling.
  ...
  // Add cashback to driver's real balance
  private async addDriverCashback(driverId: number, cargoId: string, percentage: number) {
    ...
    // Add to real balance
    await this.updateUserBalance(driverId, cashbackAmount, `${percentage}% cashback - Buyurtma #${cargoId}`);
...
💰 <b>Mavjud balans:</b> ${balance.toLocaleString()} so'm
💸 <b>Yechish miqdori:</b> ${balance.toLocaleString()} so'm
...
💡 Buyurtmalarni bajaring va cashback oling!
    ...
    const currentBalance = await this.getUserBalance(userId);
    const allTransactions = balanceHistory
      .slice(-20) // Last 20 transactions
      .reverse()
      .map((t, index) => {
        const date = new Date(t.date).toLocaleString('uz-UZ');
        const typeIcon = t.amount > 0 ? '💰' : '💸';
        return `${index + 1}. ${typeIcon} ${Math.abs(t.amount).toLocaleString()} so'm\n   ${t.reason}\n   📅 ${date}`;
      })
      .join('\n\n');

    const totalEarned = balanceHistory.filter(t => t.amount > 0).reduce((sum, t) => sum + t.amount, 0);

    const message = `
📊 <b>BATAFSIL HISOBOT</b>

💳 <b>Joriy balans:</b> ${currentBalance.toLocaleString()} so'm
📈 <b>Jami:</b> ${totalEarned.toLocaleString()} so'm
💸 <b>Yechildi:</b> 0 so'm
...
${allTransactions}
  ...
  private async showEarningsReport(ctx: any) {
    ...
    if (!balance && (!balanceHistory || balanceHistory.length === 0)) {
      await this.safeAnswerCallback(ctx, '');
    ...
    const earningTransactions = balanceHistory.filter(t => t.type === 'earning' || t.amount > 0);
    const totalEarned = earningTransactions.reduce((sum, t) => sum + Math.abs(t.amount), 0);
    const totalOrders = earningTransactions.length;
    const averageEarning = totalOrders > 0 ? Math.floor(totalEarned / totalOrders) : 0;

    const message = `
📊 <b>DAROMAD HISOBOTI</b>

💰 <b>Jami balans:</b> ${balance.toLocaleString()} so'm
📈 <b>Jami ishlab topilgan:</b> ${totalEarned.toLocaleString()} so'm
💸 <b>Yechildi:</b> 0 so'm

📋 <b>Statistika:</b>
🚛 <b>Bajarilgan buyurtmalar:</b> ${totalOrders} ta
💰 <b>O'rtacha cashback:</b> ${averageEarning.toLocaleString()} so'm
🎯 <b>Bonus tranzaksiyalar:</b> 0 ta

📅 <b>Faoliyat muddati:</b> ${balanceHistory.length > 0 ? new Date(balanceHistory[0]?.date || Date.now()).toLocaleDateString('uz-UZ') : 'Yangi'} dan
    `;
...
• 🔄 Uptime: ${Math.round(process.uptime())}s
  ...
  private async loadUserData() {
    try {
      this.logger.log(`🔄 Loading user data from: ${this.DATA_FILE_PATH}`);
      if (fs.existsSync(this.DATA_FILE_PATH)) {
        const data = JSON.parse(fs.readFileSync(this.DATA_FILE_PATH, 'utf8'));
        this.logger.log(`📄 User data file loaded successfully. Keys: ${Object.keys(data).join(', ')}`);
        ...
        // Load userBalances - KUCHAYTIRILGAN YUKLASH
        ...
        // DataService dan ham balanslarni yuklash
        try {
          const balanceData = await this.dataService.readData('balance-settings', {
            balanceSettings: { enabled: true },
            userBalances: {},
            balanceHistory: {}
          });

          if (balanceData.userBalances) {
            Object.entries(balanceData.userBalances).forEach(([userId, balance]: [string, any]) => {
              const userIdNum = parseInt(userId);
              // Agar memory da yo'q bo'lsa yoki DataService da katta bo'lsa
              if (!this.userBalances.has(userIdNum) || this.userBalances.get(userIdNum) < balance) {
                this.userBalances.set(userIdNum, balance);
              }
            });
            this.logger.log(`✅ Synchronized ${Object.keys(balanceData.userBalances).length} balances from DataService`);
          }
        } catch (dataError) {
          this.logger.warn(`⚠️ Could not load balances from DataService: ${dataError.message}`);
        }
        ...
        // Load cargoOffers
        this.logger.log(`🚛 Checking cargoOffers in data... Exists: ${!!data.cargoOffers}, Count: ${data.cargoOffers ? Object.keys(data.cargoOffers).length : 0}`);
        if (data.cargoOffers) {
          Object.entries(data.cargoOffers).forEach(([key, cargoInfo]: [string, any]) => {
            this.cargoOffers.set(key, cargoInfo);
            this.logger.log(`  ✅ Loaded cargo: ${key} - Status: ${cargoInfo.status}`);
          });
          this.logger.log(`🚛 Loaded ${Object.keys(data.cargoOffers).length} cargo offers from file. Current Map size: ${this.cargoOffers.size}`);
        } else {
          this.logger.warn(`⚠️ No cargoOffers found in user-data.json!`);
        }
        ...
        this.logger.log(`✅ Data loading completed. Final state - cargoOffers: ${this.cargoOffers.size}, userRoles: ${this.userRoles.size}`);
      } else {
        this.logger.warn(`⚠️ User data file not found: ${this.DATA_FILE_PATH}`);
      }
    } catch (error) {
...
`;
  ...
  private async clearAllUserData() {
    try {
      ...
      // Virtual balances removed - using real balance system only
...
⏰ <b>E'lon 24 soat faol!</b> Haydovchilarni kuting...
  ...
  private async showCustomerOrderHistory(ctx: any) {
    ...
    // Get all orders - both active and completed
    const allUserOrders: any[] = [];

    // Add active orders from cargoOffers
    const activeOrders = Array.from(this.cargoOffers.values())
      .filter(cargo => cargo.userId === userId);
    allUserOrders.push(...activeOrders);

    // Add completed orders from customerOrderHistory (avoid duplicates)
    const completedOrders = this.customerOrderHistory.get(userId) || [];
    const uniqueCompletedOrders = completedOrders.filter(order =>
      !allUserOrders.some(activeOrder => activeOrder.id === order.id)
    );
    allUserOrders.push(...uniqueCompletedOrders);
...
💡 Yuk berish uchun "📦 Yuk berish" tugmasini bosing.
    ...
    for (let i = 0; i < Math.min(sortedOrders.length, 5); i++) {
      ...
      const statusText = order.status === 'active' ? 'Haydovchi izlanmoqda' :
                        order.status === 'matched' ? 'Haydovchi topildi' :
    ...
    if (allUserOrders.length > 5) {
      ordersText += `📄 <b>Va yana ${allUserOrders.length - 5} ta order...</b>\n\n`;
    }
...
• Masofaviy cheklash: ❌ O'chirilgan
  ...
  private async getUserBalance(userId: number): Promise<number> {
    try {
      ...
      // 2. DataService orqali balance-settings.json dan olish
      try {
        const dataServiceBalance = await this.dataService.getUserBalance(userId.toString());
        if (dataServiceBalance > 0) {
          // DataService balansni memory ga ko'chirish
          this.userBalances.set(userId, dataServiceBalance);
          this.logger.log(`💾 Balance from DataService for user ${userId}: ${dataServiceBalance}, moved to memory`);
          return dataServiceBalance;
        }
      } catch (dataError) {
        this.logger.warn(`⚠️ DataService balance read error: ${dataError.message}`);
      }

      // 3. To'g'ridan-to'g'ri fayl ichidan olish (backup)
      const filePath = path.join(process.cwd(), 'src', 'data', 'balance-settings.json');
      if (fs.existsSync(filePath)) {
        const data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
...
⚠️ <b>Eslatma:</b> Narxni o'zgartirish darhol qo'llaniladi.
  ...
  // Balans yangilash funksiyasi - KUCHAYTIRILGAN SAQLASH
  private async updateUserBalance(userId: number, amount: number, reason: string = 'Balance update') {
    ...
    // 2. Asosiy fayl (user-data.json) ga saqlash
    await this.savePaymentSettingsToFile();

    // 3. DataService orqali balance-settings.json ga ham saqlash
    try {
      await this.dataService.setUserBalance(userId.toString(), newBalance);
      ...
      this.logger.log(`💾 Balance saved to DataService: ${userId} -> ${newBalance}`);
    } catch (error) {
      this.logger.error(`❌ DataService balance save error: ${error.message}`);
    }
  ...
  public async getDashboardOrders(status?: string, limit?: number) {
    try {
      this.logger.log(`🔍 getDashboardOrders called with status: ${status}, limit: ${limit}`);

      // To'g'ridan-to'g'ri user-data.json faylidan o'qish
      let orders = [];

      try {
        if (fs.existsSync(this.DATA_FILE_PATH)) {
          const data = JSON.parse(fs.readFileSync(this.DATA_FILE_PATH, 'utf8'));
          this.logger.log(`📄 Reading fresh data from disk. Available keys: ${Object.keys(data).join(', ')}`);

          if (data.cargoOffers) {
            const cargoArray = Object.values(data.cargoOffers);
            this.logger.log(`🚛 Found ${cargoArray.length} cargo offers in user-data.json`);

            // Faqat faol zakazlarni dashboard'da ko'rsatish (completed zakazlar History'da bo'ladi)
            const activeCargoArray = cargoArray.filter((cargo: any) =>
              cargo.status !== 'completed' && cargo.status !== 'cancelled'
            );
            this.logger.log(`📊 Filtered to ${activeCargoArray.length} active orders (excluding completed/cancelled)`);

            // Dashboard format ga o'tkazish
            orders = activeCargoArray.map((cargo: any) => ({
              id: cargo.id,
            ...
            this.logger.log(`✅ Converted ${orders.length} orders to dashboard format`);
          } else {
            this.logger.warn(`⚠️ No cargoOffers found in user-data.json!`);
          }
        } else {
          this.logger.error(`❌ user-data.json file not found at: ${this.DATA_FILE_PATH}`);
        }
      } catch (diskError) {
        this.logger.error(`❌ Error reading from disk: ${diskError.message}`);
        // Fallback to memory Map
        orders = Array.from(this.cargoOffers.values());
        this.logger.log(`🔄 Fallback to memory Map: ${orders.length} orders`);
      }

      this.logger.log(`📊 getDashboardOrders - Processing ${orders.length} total orders`);
      ...
      if (status && status !== 'all') {
        ...
        this.logger.log(`🔍 Filtered orders by status '${status}': ${originalCount} -> ${orders.length}`);
      }
      ...
      this.logger.log(`🔍 Processing ${orders.length} orders for dashboard. First 3 orders status: ${orders.slice(0,3).map(o => `${o.id}:${o.status}`).join(', ')}`);
      ...
      return orders.map(order => {
        ...
        if (result.id && result.id.includes('debug')) {
          this.logger.log(`🐛 Debug order mapped: ${result.id} status: ${result.status}`);
        }
  ...
  public async createOrderFromDashboard(orderData: any) {
    try {
      ...
      let customerId = 0;
      let customerName = 'Admin Dashboard';

      if (orderData.customerPhone) {
        // Clean phone number
        const cleanPhone = orderData.customerPhone.replace(/[^0-9]/g, '');
        this.logger.log(`🔍 Searching customer by phone: ${orderData.customerPhone} -> cleaned: ${cleanPhone}`);

        // Search for existing customer by phone
        for (const [userId, userRole] of this.userRoles.entries()) {
          if (userRole.role === 'yukchi' && userRole.profile) {
            // Extract all phone numbers from profile
            const profilePhone = userRole.profile.phone || '';
            const profileCompany = userRole.profile.company || '';
            const profileContact = userRole.profile.contact || '';

            // Clean all phone-like strings
            const userPhones = [profilePhone, profileCompany, profileContact]
              .map(field => (field || '').replace(/[^0-9]/g, ''))
              .filter(phone => phone.length >= 12); // Valid phone length

            if (userPhones.some(phone => phone === cleanPhone || phone.endsWith(cleanPhone.slice(-9)))) {
              customerId = userId;
              customerName = userRole.profile.name || userRole.profile.fullName || userRole.profile.contact || 'Mijoz';
              this.logger.log(`✅ Found customer: ${customerId} - ${customerName} for phone ${cleanPhone}`);
              break;
            }
          }
        }
        ...
        if (customerId === 0) {
          this.logger.log(`❌ Customer not found for phone: ${cleanPhone}`);
          this.logger.log(`📋 Available customers: ${Array.from(this.userRoles.entries())
            .filter(([_, role]) => role.role === 'yukchi')
            .map(([id, role]) => `${id}: ${role.profile?.phone || 'no phone'}`)
            .join(', ')}`);
        }
      }
      ...
      const cargoOffer = {
        ...
        phone: orderData.customerPhone || orderData.phone || 'Admin',
      ...
      // Add to cargo offers
      this.cargoOffers.set(cargoId, cargoOffer);
      this.logger.log(`✅ Added cargo to cargoOffers: ${cargoId}. Total cargo offers: ${this.cargoOffers.size}`);

      // Save data to file immediately and verify
      await this.saveUserData();
      this.logger.log(`💾 User data saved after creating cargo: ${cargoId}`);

      // Verify the order was saved correctly
      this.logger.log(`✅ Current cargoOffers size: ${this.cargoOffers.size}`);
...
💡 **Qabul qilgandan keyin mijoz raqamini ko'rasiz**`;
  ...
  async cancelOrderFromDashboard(orderId: string) {
    try {
      ...
      if (this.customerOrderHistory.has(cargoOffer.userId)) {
        ...
        if (orderIndex !== -1) {
          customerOrders[orderIndex].status = 'cancelled';
          customerOrders[orderIndex].cancelledDate = new Date().toISOString();
        }
      }
  ...
  async resendOrderFromDashboard(orderId: string) {
    try {
      ...
      // Update customer order history status back to active
      if (this.customerOrderHistory.has(cargoOffer.userId)) {
        const customerOrders = this.customerOrderHistory.get(cargoOffer.userId)!;
        const orderIndex = customerOrders.findIndex(order => order.id === orderId);
        if (orderIndex !== -1) {
          customerOrders[orderIndex].status = 'active';
          delete customerOrders[orderIndex].cancelledDate;
          delete customerOrders[orderIndex].completedDate;
        }
      }
  ...
  // Method to read completed orders history
  private async getCompletedOrdersHistory(): Promise<any[]> {
    try {
      ...
      this.logger.error('❌ Error reading order history:', error);
      return [];
    }
  }
...
👋 Xush kelibsiz! Quyidagi xizmatlardan foydalaning:`;
  ...
  // Get balance history for user
  private async getBalanceHistory(userId: number): Promise<any[]> {
    try {
      const filePath = path.join(process.cwd(), 'src', 'data', 'balance-settings.json');
      if (fs.existsSync(filePath)) {
        const data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
        return data.balanceHistory?.[userId] || [];
      }
    } catch (error) {
      this.logger.error('❌ Error loading balance history:', error);
    }
